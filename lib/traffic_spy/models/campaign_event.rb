module TrafficSpy
  class CampaignEvent
    ...
    def self.campaign_event_count(identifier)
      ...
      campaigns_for_account = campaign_events.
      where(:account_id => account_id).
      group_and_count(:campaign_id).order(Sequel.desc(:count))
    ...
    def self.campaign_events(identifier, campaign)
      ...
      events_for_campaign = campaign_events.where(:account_id => account_id).
      where(:campaign_id => campaign_id).order(Sequel.asc(:id))
    ...
    def self.get_id(identifier)
      campaign_events_row = DB[:campaign_events].
      where(:campaign_id => campaign_id).where(:event_id => event_id).to_a
